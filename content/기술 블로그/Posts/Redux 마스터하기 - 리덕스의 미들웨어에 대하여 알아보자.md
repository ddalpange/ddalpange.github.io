...
modified: 2019-01-11
원문 링크: https://ddalpange.github.io/blog/2019-01-11-redux-step2
카테고리: Engineering
---
...
기존 `action -> addTodo`는 단순한 **객체**를 리턴했지만 이제 비동기를 처리하기 위해 **dispatch를 인자로 받고 그에 대한 작업을 promise로 반환하는 함수**를 리턴해야한다. `redux-thunk`는 단순히 `action`에서 반환되어온 함수에 `dispatch`를 인자로 넣어 실행할 뿐이다. `redux-thunk`를 적용한 코드는 아래와 같다.
...
기본적인 `CRUD` 작업을 완료하였다. 다음 포스트에서는 `react`와 `react-redux`를 이용해 뷰를 만드는 작업을 진행한다. CodeSandBox에 예제를 올려두었으니 참고하면 된다.

---

원문: [https://ddalpange.github.io/blog/2019-01-11-redux-step2](https://ddalpange.github.io/blog/2019-01-11-redux-step2)