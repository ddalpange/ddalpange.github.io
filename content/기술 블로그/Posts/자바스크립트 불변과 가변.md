...
base: "[[Untitled.base]]"
작성일: 2017-10-03
원문 링크: https://ddalpange.github.io/blog/2017-10-03-js-immutable-mutable
카테고리: Engineering
---
...
3. 값을 변경하기 위하여 다음의 과정을 거친다. 새로운 인스턴스를 생성한다.
4. 기존의 인스턴스의 포인터를 지운다.
5. 포인터에 새로운 인스턴스의 주소를 할당한다.
6. null과 undefined는 숫자도 문자도 불리언 값도 아닌 독립적인 값에 속한다.
...
따라서 ***let stringExample2 = stringExample*** 이라는 명령어를 실행할 때.

**객체와 인스턴스의 관계가 1:1로 형성됩니다.**

7. 값을 변경할 수 있다.
8. Array, List, Map
9. 값을 변경할 때 인스턴스 자체가 변한다.
...
따라서 ***arrayExample = arrayExample2*** 명령을 실행한다면
...
10. 기존의 가변형 객체였던 Map, List 등을 불변형으로 쓸 수 있게 해줍니다.
...
11. Front에서는 사용자에게 보여줄 값과, 실제 데이터값을 분리할 필요가 있다.
12. shouldComponentUpdate같은 중요한 메소드를 사용할 때, 기준이 되는 변수가 함수 A,B를 거쳐왔는지 A,C를 거쳐왔는지 장담할 수 없다.
13. 객체 배열을 Shallow copy 해버린다면 각각의 데이터에 독립성을 보장할 수 없다.
14. Immutable.js, cloneObject 등 Deep copy를 하게된다면 데이터의 독립성을 보장하며, 명시적이며, 유지보수를 쉽게 할 수 있다.

---

원문: [https://ddalpange.github.io/blog/2017-10-03-js-immutable-mutable](https://ddalpange.github.io/blog/2017-10-03-js-immutable-mutable)